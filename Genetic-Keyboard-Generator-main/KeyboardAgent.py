...
def precalculateKeyDistances(agent):
    ...
    for (keyA, posA) in agent.keymap.items():
        for (keyB, posB) in agent.keymap.items():
            keyDistances[(posA[0], posA[1], posB[0], posB[1])] = math.sqrt(
...
class KeyboardAgent:
    ...
    def calculateFitness(self):
        ...
        # if self.keymap['f'][0] != 3.25 or self.keymap['c'][1] != 2:
        #     return # force position of a
        #     print("F")
        # elif self.keymap['j'][0] != 6.25 or self.keymap['v'][1] != 2:
        #     self.fitness += 1
        #     print("J")
    ...
    def calculateFitness(self):
        ...
        for (key, frequency) in singleKeyFreqs.items():
            keyPos = self.keymap[key]
            if self.keymap['j'][0] != 6.25:
                return  # force position of j
                print("J")

            if self.keymap['f'][0] != 3.25:
                return  # force position of f
                print("F")
